...
class PointCollectorApp:
    ...
    def plot_spline(self):
        ...
        for i in range(len(self.spline_points)):
            ...
            ts_plot = np.arange(min(xs), max(xs) + STEP, STEP)
            ys_plot = cs(ts_plot)

            self.ax.plot(xs, ys, 'o', label=f'Points {i + 1}')
            self.ax.plot(ts_plot, ys_plot, linewidth=2, label=f'Spline {i + 1}')
