...
class PointCollectorApp:
    def __init__(self, root, image_path):
        ...
        self.fig, self.ax = plt.subplots(figsize=(self.canvas.winfo_width() / 100, self.canvas.winfo_height() / 100))
    ...
    def add_spline(self):
        ...
        self.plot_spline()
        self.save_points_to_file(len(p))

    def save_points_to_file(self,ts_n):
        for i, points in enumerate(self.spline_points):
            ...
            with open(filename, "w") as file:
                for j,point in enumerate(points):
                    file.write(f"{point[0]} {point[1]} {ts[i]}\n")
