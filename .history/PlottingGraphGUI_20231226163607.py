...
from Settings import image_path, line_thickness, colour
STEP = 0.001
...
class PointCollectorApp:
    ...
    def save_points_to_file(self):
        for i, points in enumerate(self.spline_points):
            filename = f"data/spline_{i + 1}_points.txt"
            n = len(points) - 1
    ...
    def plot_spline(self):
        ...
        for i in range(len(self.spline_points)):
            ...
            self.ax.plot(xs_plot, ys_plot, linewidth=line_thickness,color=colour)
...
root.mainloop()
app.save_image("data/output_plot.png")
