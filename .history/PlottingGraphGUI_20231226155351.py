...
class PointCollectorApp:
    ...
    def on_canvas_resize(self, event):
        """Avoids graph squashing when a new spline is added
        """
    ...
    def plot_spline(self):
        """For each set of points (x,y) creates a cubic spline function
            and plots it
        """
        self.ax.clear()
        for i in range(len(self.spline_points)):
            ...
            if self.spline_points and self.spline_points[-1] == self.spline_points[i]:
    # Plot points in green for the last spline added
                self.ax.plot(xs, ys, 'go', markersize=8)

            self.ax.plot(xs_plot, ys_plot, linewidth=5)
